...
(function() {
  ...
  sumBkt = function(bkts) {
    return bkts.map(bkt => {
      return bkt.reduce((sum, val) => sum += val[1], 0);
    });
  };
  ...
  bucketize = function(points, numBkts, minX, diffX, minY, maxY, h, bucketAgg) {
    var bkt, diff, max, min, _ref;
    const bkts = createBkt(points, numBkts, minX, diffX);
    bkt = bucketAgg === 'sum' ? sumBkt(bkts) : avgBkt(bkts);
  ...
  module.exports = function(points, options) {
    ...
    width -= lblYW;
    numBkts = 145; // Math.min(uniqueX, width - lblYW);

    bktW = Math.floor((width - lblYW) / numBkts);
    if (xFractions == null) {
      xFractions = minMax(0, 8, Math.log(numBkts / diffX * 5) / Math.LN10);
    }
    _ref7 = bucketize(points, numBkts, minX, diffX, minY, maxY, height, options.bucketAgg), bkt = _ref7.bkt, min = _ref7.min, max = _ref7.max, diff = _ref7.diff;
    lblY = [];
